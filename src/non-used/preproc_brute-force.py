...
# load important features, if they already stored
INPUT_ADDED = '..\\input_added'
importance_file = f'{INPUT_ADDED}/brute_force_important.csv'
importance_from = None
if os.path.isfile(importance_file): 
    importance_from = pd.read_csv(importance_file) # only important features
...
for coupling_type in types:
    cur_x =  X[X.type == coupling_type]
    cur_x = cur_x.drop(['type'], axis=1)
    cur_x.to_csv(f'{INPUT_ADDED}/brute_X_{coupling_type}.csv')      

    cur_x_test =  X_test[X_test.type == coupling_type]
    cur_x_test = cur_x_test.drop(['type'], axis=1)
    cur_x_test.to_csv(f'{INPUT_ADDED}/brute_X_test_{coupling_type}.csv')      


    #X.to_csv(f'{INPUT_ADDED}/brute_force_X.csv')
    #X_test.to_csv(f'{INPUT_ADDED}/brute_force_X_test.csv')
    #important_features.to_csv(f'{INPUT_ADDED}/brute_force_important.csv')